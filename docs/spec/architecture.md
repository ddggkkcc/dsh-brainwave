...
│        │    - 临时追问（问题输入 + 是否加上下文开关）                               │
│        ▼                                                                         │
│  ctx.remote.brainwave.{list, save, remove, ask}   ◀──── 远程调用                  │
└──────────────────────────────┬───────────────────────────────────────────────────┘
                               │
┌──────────────────────────────▼────────────────────── 宿主（host，Node）───────────┐
│                                                                                  │
│  [remote 命名空间]  brainwave（TypertRemoteService）                               │
│        │  list / save / remove / ask                                             │
│        ├──► [存储]  storageDomain（灵感条目持久化）                                │
...
| 模块 | 职责 | 依赖 |
|---|---|---|
| `remote` | 注册 `brainwave` remote 命名空间，暴露 `list/save/remove/ask` | `typert-protocol`、`storageDomain`、`llm` |
| `storage` | 灵感条目持久化（schema 校验） | `storage-domain` |
...
| `overlay-panel` | `shell.overlay` 浮窗（灵感库列表 + 追问） | `slots`、`remote` |
| `store` | 客户端内存态（灵感列表缓存、当前选区、追问配置） | — |
...
2. 浮钮「收藏」→ `remote.brainwave.save({ text, source })`。
...
3. 客户端 `remote.brainwave.ask({ query, includeContext })`。
...
| 远程 | 宿主自注册 remote 命名空间 `brainwave` |
